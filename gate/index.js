...
/** Attribution footer appended to every generated audit report. The report is
 *  bilingual (English half, then `<!-- REPORT_ZH -->`, then Chinese half) and
 *  the panel splits on that marker, so a single footer at the very end would
 *  only appear in the Chinese view. Append the English footer at the end of the
 *  English half (just before the marker) and the Chinese footer at the very
 *  end, so BOTH language views carry the attribution. */
const REPORT_FOOTER_EN =
  '\n\n---\n\n' +
  ...
  async function runScanLlm(info, scan, state, reportDir, { signal }) {
    ...
    try {
      ...
      if (isInsideReports(reportsRoot, reportDir)) {
        // Insert the English footer at the end of the English half (right
        // before the bilingual marker) so it survives the panel's split; the
        // Chinese footer goes at the very end. A report without the marker
        // (single-language) gets the English footer only.
        const marker = '<!-- REPORT_ZH -->';
        const withFooters = report.includes(marker)
          ? report.replace(marker, REPORT_FOOTER_EN + '\n\n' + marker) + REPORT_FOOTER_ZH
          : report + REPORT_FOOTER_EN;
        writeFileSync(join(reportDir, 'report.md'), sanitizeReportHtml(withFooters), { mode: 0o600 });
